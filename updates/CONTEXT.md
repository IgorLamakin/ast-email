...
4. Опубликовать (токен нужен из-за приватного репозитория):
   ```
   cd electron
   $env:GH_TOKEN = "..."          # Personal Access Token с правами repo
   npm run publish
   ```
   В GitHub поднимется Release `v2.3.0` с установщиком и `latest.yml`.
5. При желании отредактировать описание релиза (`gh release edit`).

## 6. Известные ограничения / что нужно для production

- **Приватный репозиторий**: electron-updater скачает обновление, только если у
  приложения есть токен GitHub с доступом к `IgorLamakin/ast-email`. Энд
  юзеры токена иметь не будут → вариант б) или публичный релиз-repo:
  а) перевести репозиторий/сделать отдельный публичный релизы-репо;
  б) поднять «generic» HTTP-провайдер (простая папка с `latest.yml` +
     установщиком на внутреннем сервере) — токен не нужен.
  Рекомендуется б) для внутренней раздачи либо для публичного релиза.
